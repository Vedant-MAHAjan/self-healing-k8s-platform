...
class Settings(BaseSettings):
    ...
    enable_autonomous_control: bool = Field(
        default=True,
        description="Enable policy-driven autonomous orchestration"
    )

    control_policy_path: Optional[str] = Field(
        default=None,
        description="Path to the control policy YAML file"
    )

    state_store_path: str = Field(
        default="/tmp/self-healing-control.db",
        description="SQLite database path for persistent control state"
    )

    scheduler_poll_interval: int = Field(
        default=5,
        description="Seconds between scheduler polling cycles"
    )

    scheduler_worker_concurrency: int = Field(
        default=2,
        description="Number of concurrent remediation workers"
    )

    metrics_window_minutes: int = Field(
        default=15,
        description="Default metrics lookback window in minutes"
    )
